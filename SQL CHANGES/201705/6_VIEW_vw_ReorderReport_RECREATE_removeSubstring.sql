...
SELECT 
	...
	CASE WHEN ISNULL(DATEDIFF(DAY ,a.[Date product was last stocked],a.[Date Last Sale]),0) = 0 
	       THEN 0
		   ELSE CAST(CAST(A.[Total Number of Units Sold between Last Stock and Last sale date]  AS DECIMAL(18,2)) / ISNULL(DATEDIFF(DAY ,a.[Date product was last stocked],a.[Date Last Sale]) + 1,0) AS DECIMAL(18,2))  
...
LEFT JOIN [PeopleSource].[dbo].order_report o ON o.item_SKU  --LIKE '%'+SUBSTRING(final.SKU,0,30) +'%' 
	LIKE '%'+ final.SKU +'%' AND  [item_sku] <>'' AND order_source NOT IN ('ret','rma')
	AND (
