...
/****** Object:  View [dbo].[vw_ReorderProducts]    Script Date: 2/27/2018 9:12:43 PM ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	ISNULL(CAST(A.[Quantity on Hand] AS INT),0) AS Quantity,
		...
		(
			...
			LEFT JOIN (	
				...
				FROM( 
					...
					WHERE [item_sku]<>'' 
					AND ([item_sku] LIKE '%AAA-M1%'	OR [item_sku] LIKE '%AAA-K1%' OR [item_sku] LIKE '%AAA-W1%')
			...
			(SKU LIKE '%AAA-M1%' OR SKU LIKE '%AAA-K1%' OR SKU LIKE '%AAA-W1%')  AND r=1
	 )final
